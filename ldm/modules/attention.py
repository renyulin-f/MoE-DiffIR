...
def default(val, d):
    if exists(val):
        return val  #(3,7,1024)
    return d() if isfunction(d) else d #(3,1024,320)
...
class GEGLU(nn.Module):
    ...
    def forward(self, x):
        x, gate = self.proj(x).chunk(2, dim=-1)  #【3,1024,320】-> [3,1024,1280], [3,1024,1280]
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        h = self.heads
        #报错：x=[2,1024,320] context=[4,77,1024]
        q = self.to_q(x)
        context = default(context, x)  #（3,1024,320）
...
class SpatialTransformerV2(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head,
        ...
        inner_dim = n_heads * d_head #64*5 =320
        self.norm = Normalize(in_channels)
        if not use_linear:
            self.proj_in = nn.Conv2d(in_channels,
                                     inner_dim,
    ...
    def forward(self, x, context=None): # context=(4,77,1024)
        # note: if no context is given, cross-attention defaults to self-attention
        ...
        if not self.use_linear:
            x = self.proj_in(x)
        x = rearrange(x, 'b c h w -> b (h w) c').contiguous()   # (4,320,32,32) (4,1024,320)
        if self.use_linear:
            x = self.proj_in(x)
