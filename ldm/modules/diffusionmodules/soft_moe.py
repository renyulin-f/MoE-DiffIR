...
class RMSNorm(Module):
    def __init__(self, dim):
        super().__init__()
        self.scale = dim ** 0.5   #256的根号为16
        self.gamma = nn.Parameter(torch.ones(dim)) #256的一个可学的向量
...
class Experts(nn.Module):
    ...
    def forward(
        ...
        x = rearrange(x, 'b e n d -> e b n d')   #(1,8,128,256)->(8,1,128,256)
        ...
        for expert, expert_input in zip(experts, x):
            out = expert(expert_input) #(1,256,512)
            outs.append(out)
...
class SoftMoE(Module):
    def __init__(
        ...
        *,
        ...
        num_slots = default(num_slots, seq_len // num_experts) #1024/128 = 8 意思是1024个token会被8个num_prompts给平分。 即每个prompt都会负责128个token。

        norm_klass = LayerNorm if use_layernorm else RMSNorm
        self.norm = norm_klass(dim)

        self.slot_norm = norm_klass(dim)
        self.slot_embeds = nn.Parameter(torch.randn(num_experts, num_slots, dim))  #num_experts=8, num_slots=128,dim =256
    ...
    def forward(self, x, mask = None, add_noise = False, noise_mult = 1.):
        ...
        x = self.norm(x) #（1，1024，256）
        slot_embeds = self.slot_norm(self.slot_embeds) # 8,128,256

        logits = einsum('b n d, e s d -> b n e s', x, slot_embeds) # 1,1024,8,128  soft_moe weighting logits
        ...
        dispatch_weights = logits.softmax(dim = 1) #(1,1024,8,128)   soft per slot , 主要是针对每一列进行softmax

        combine_weights = rearrange(logits, 'b n e s -> b n (e s)') #(1,1024,1024)   soft per token,   normalized per row.
        ...
        slots = einsum('b n d, b n e s -> b e s d', x, dispatch_weights) # x=(1,1024,256)  dis=(1,1024,8,128)
