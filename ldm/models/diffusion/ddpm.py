...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
                 ...
                 loss_type="l2",
                #  ckpt_path='/data2/renyulin/StableSR/v2-1_512-ema-pruned.ckpt',
                 ckpt_path=None,
                 ignore_keys=[],
        ...
        self.model = DiffusionWrapper(unet_config, conditioning_key)  #这里是u-net model的构建
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, moe_prompt_cond=None, seg_cond=None):
        if self.conditioning_key is None:
            out = self.diffusion_model(x, t)
        elif self.conditioning_key == 'concat': #这里有concat形式
            xc = torch.cat([x] + c_concat, dim=1)
            out = self.diffusion_model(xc, t)
        elif self.conditioning_key == 'crossattn':
            cc = torch.cat(c_crossattn, 1)
            if seg_cond is None:
                out = self.diffusion_model(x, t, context=cc, moe_prompt_cond=moe_prompt_cond)
            else:
                out = self.diffusion_model(x, t, context=cc, moe_prompt_cond=moe_prompt_cond, seg_cond=seg_cond)
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x):
        x = x + self.attn(self.norm1(x))  #(4,256,32,32)
...
class PromptLatentDiffusion(DDPM): #SRTextWT
    """main class"""
    def __init__(self,
                 first_stage_config,
                 cond_stage_config,
                 moe_prompt_module_config,
        ...
        if self.cond_stage_model.Type=='semantic':
            self.transformer_encoder = nn.Sequential(nn.Conv2d(4, embed_dim, 3, 1, 1),
            TransformerBlock(dim=256, num_heads=1, ffn_expansion_factor=2.66, bias=False, LayerNorm_type='WithBias'),
            TransformerBlock(dim=256, num_heads=1, ffn_expansion_factor=2.66, bias=False, LayerNorm_type='WithBias')
            )
            self.transformer_decoder = nn.Sequential(TransformerBlock(dim=256, num_heads=1, ffn_expansion_factor=2.66, bias=False, LayerNorm_type='WithBias'),
            TransformerBlock(dim=256, num_heads=1, ffn_expansion_factor=2.66, bias=False, LayerNorm_type='WithBias'),
            nn.Conv2d(256,4,1,1)
            )
        ...
        #这里是设置sef.model(unet)为frozen
        if not self.unfrozen_diff:
            self.model.eval()
            # self.model.train = disabled_train
            for name, param in self.model.named_parameters():
                if 'spade' not in name:  ## Finetune only the spade layer.
    ...
    def instantiate_cond_stage(self, config):  #Text Encoder
    ...
    def instantiate_moe_prompt_stage(self, config):
        model = instantiate_from_config(config)
    ...
    def _dequeue_and_enqueue(self):
        ...
        if b == self.configs.data.params.batch_size:
            if not hasattr(self, 'queue_size'): #判断一个对象是否拥有该属性
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        #这里输入image semantic information
        if self.cond_stage_model.Type =='text':
            out = [z, text_cond]
        else:
            c = resize(self.lq)
            out = [z, c]
        out.append(z_gt)
        out.append(resize(self.lq))
        return out # out=[z_lq, text_cond(or visual_cond), z_gt, lq]
    ...
    def shared_step(self, batch, **kwargs):
        img_lq, img_gt, task_id = batch
        batch = [img_lq,img_gt] # img_lq: (B,C,H,W), img_gt: (B,C,H,W)
        x, c, gt, lq = self.get_input(batch, self.first_stage_key) #这里的c是text input, x是lq_z(latent),gt是gt_z(latent)
        if self.cond_stage_model.Type=='semantic':
            x_in  = x
            x_prompt = self.transformer_encoder(x_in)
            x2 = self.transformer_decoder(x_prompt)
            x_out = x_in + x2
            c = resize(self.decode_first_stage(x_out))
        loss, loss_dict = self(x, c, gt,task_id,lq)
        return loss, loss_dict
    
    def shared_step2(self, batch, **kwargs):
        x, c, gt = self.get_input(batch, self.first_stage_key) #这里的c是text input, x是lq_z(latent),gt是gt_z(latent)
        loss = self(x, c, gt)
        return loss

    def calculate_color_loss(self,gt, sr):
    # 转换为YUV颜色空间
        gt_y = 0.299 * gt[:, 0, :, :] + 0.587 * gt[:, 1, :, :] + 0.114 * gt[:, 2, :, :]
        sr_y = 0.299 * sr[:, 0, :, :] + 0.587 * sr[:, 1, :, :] + 0.114 * sr[:, 2, :, :]

        # 计算Y通道差异
        color_loss = torch.abs(gt_y - sr_y)

        # 返回Y通道损失
        return color_loss.mean(dim=(1, 2))  # 在批处理维度上求均值

#开始修改
    def forward(self, x, c, gt,clean_id,lq,*args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable: #CLIP是被fixed住的
                c = self.get_learned_conditioning(c)
            else:
                c = self.cond_stage_model(c)  #(3,7,1024)

        struc_c = self.structcond_stage_model(x,clean_id,lq) # [64,32,16,8]四个维度上得到的特征
    ...
    def apply_model(self, x_noisy, t, cond, moe_prompt_cond, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            cond['moe_prompt_cond'] = moe_prompt_cond
            x_recon = self.model(x_noisy, t, **cond) #这里把 cond(cross_atten的text和moe_prompt_cond输入进去)
    ...
    def p_losses(self, x_start, cond, moe_prompt_cond, t, t_ori, z_gt, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start)) #cond=text
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise) #x_start=gt
        #得到x_t=前向加噪
        if self.mix_ratio > 0:
            if random.random() < self.mix_ratio:
                ...
                x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        #关键部分：得到整个模型的输出：需要输入 (x_t,t,text_condition,以及训练的time_aware_condition)
        model_output = self.apply_model(x_noisy, t_ori, cond, moe_prompt_cond)
        ...
        # loss_color = self.calculate_color_loss(model_output_, target)
        ...
        # loss += loss_color*0.01
        loss_dict.update({f'{prefix}/loss': loss})

        return loss, loss_dict

    def p_mean_variance(self, x, c, moe_prompt_cond, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
                        return_x0=False, score_corrector=None, corrector_kwargs=None, t_replace=None):
        if t_replace is None:
            t_in = t
        else:
            t_in = t_replace
        model_out = self.apply_model(x, t_in, c, moe_prompt_cond, return_ids=return_codebook_ids)
    ...
    def p_mean_variance_canvas(self, x, c, moe_prompt_cond, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
        ...
        for row in range(grid_rows):
            ...
            for col in range(grid_cols):
                ...
                cond_tile = moe_prompt_cond[:, :, input_start_y:input_end_y, input_start_x:input_end_x]
                ...
                if len(input_list) == batch_size or col == grid_cols-1:
                    ...
                    moe_prompt_cond_input = self.structcond_stage_model(cond_list, t_in[:input_list.size(0)])
                    model_out = self.apply_model(input_list, t_in[:input_list.size(0)], c[:input_list.size(0)], moe_prompt_cond_input, return_ids=return_codebook_ids)
    ...
    @torch.no_grad()
    def p_sample(self, x, c, moe_prompt_cond, t, clip_denoised=False, repeat_noise=False,
                 return_codebook_ids=False, quantize_denoised=False, return_x0=False,
                 temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None, t_replace=None):
        b, *_, device = *x.shape, x.device
        outputs = self.p_mean_variance(x=x, c=c, moe_prompt_cond=moe_prompt_cond, t=t, clip_denoised=clip_denoised,
    ...
    @torch.no_grad()
    def p_sample_canvas(self, x, c, moe_prompt_cond, t, clip_denoised=False, repeat_noise=False,
                 return_codebook_ids=False, quantize_denoised=False, return_x0=False,
                 temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None, t_replace=None,
                 tile_size=64, tile_overlap=32, batch_size=4, tile_weights=None):
        b, *_, device = *x.shape, x.device
        outputs = self.p_mean_variance_canvas(x=x, c=c, moe_prompt_cond=moe_prompt_cond, t=t, clip_denoised=clip_denoised,
                                       return_codebook_ids=return_codebook_ids,
                                       quantize_denoised=quantize_denoised,
                                       return_x0=return_x0,
                                       score_corrector=score_corrector, corrector_kwargs=corrector_kwargs, t_replace=t_replace,
    ...
    @torch.no_grad()
    def progressive_denoising(self, cond, moe_prompt_cond, shape, verbose=True, callback=None, quantize_denoised=False,
        ...
        for i in iterator:
            ...
            img, x0_partial = self.p_sample(img, cond, moe_prompt_cond, ts,
                                            clip_denoised=self.clip_denoised,
    ...
    @torch.no_grad()
    def p_sample_loop(self, cond, moe_prompt_cond, shape, return_intermediates=False,
            ...
            if t_replace is not None:
                if start_T is not None:
                    if self.ori_timesteps[i] > start_T:
                         continue
                moe_prompt_cond_input = self.structcond_stage_model(moe_prompt_cond[0],moe_prompt_cond[1],moe_prompt_cond[2])
            else:
                if start_T is not None:
                    if i > start_T:
                        continue
                moe_prompt_cond_input = self.structcond_stage_model(moe_prompt_cond[0],moe_prompt_cond[1],moe_prompt_cond[2])
            img = self.p_sample(img, cond, moe_prompt_cond_input, ts,
                                clip_denoised=self.clip_denoised,
    ...
    @torch.no_grad()
    def p_sample_loop_canvas(self, cond, moe_prompt_cond, shape, return_intermediates=False,
                      x_T=None, verbose=True, callback=None, timesteps=None, quantize_denoised=False,
                      mask=None, x0=None, img_callback=None, start_T=None,
        ...
        for i in iterator:
            ...
            img = self.p_sample_canvas(img, cond, moe_prompt_cond, ts,
                                clip_denoised=self.clip_denoised,
                                quantize_denoised=quantize_denoised, t_replace=t_replace,
                                tile_size=tile_size, tile_overlap=tile_overlap, batch_size=batch_size, tile_weights=tile_weights)
    ...
    @torch.no_grad()
    def sample(self, cond, moe_prompt_cond, batch_size=16, return_intermediates=False, x_T=None,
    ...
    @torch.no_grad()
    def sample_canvas(self, cond, moe_prompt_cond, batch_size=16, return_intermediates=False, x_T=None,
               verbose=True, timesteps=None, quantize_denoised=False,
    ...
    @torch.no_grad()
    def sample_log(self,cond,moe_prompt_cond,batch_size,ddim, ddim_steps,**kwargs):

        if ddim:
            raise NotImplementedError

        else:
            samples, intermediates = self.sample(cond=cond, moe_prompt_cond=moe_prompt_cond, batch_size=batch_size,
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        img_lq , img_gt, task_id = batch
        # task_id = task_id[0].cpu()
        batch = [img_lq, img_gt]
        log = dict()
        z, c_lq, z_gt,x, gt, yrec, xc, lq = self.get_input_val(batch, self.first_stage_key,
        ...
        if self.test_gt:
            ...
            log["recon_lq"] = self.decode_first_stage(z1)

        if self.cond_stage_model.Type=='semantic':
            z2 = self.transformer_encoder(z)
            # z2 = self.prompt_model(z2,task_id)
            z2 = self.transformer_decoder(z2)
            z2 = resize(self.decode_first_stage(z2+z))
            c = self.cond_stage_model(z2)
        else:
            c = self.cond_stage_model(c_lq)
            
        if self.test_gt:
            moe_prompt_cond = z_gt
        else:
            moe_prompt_cond = z
        ...
        if sample:
            ...
            with self.ema_scope("Plotting"):
                if self.time_replace is not None:
                    ...
                    cur_time_step = 1000
                samples, z_denoise_row = self.sample(cond=c, moe_prompt_cond=[moe_prompt_cond,task_id,resize(lq)], batch_size=N, timesteps=cur_time_step, return_intermediates=True, time_replace=self.time_replace)
            x_samples = self.decode_first_stage(samples)
            log["samples"] = x_samples

        if plot_progressive_rows:
            with self.ema_scope("Plotting Progressives"):
                img, progressives = self.progressive_denoising(c, moe_prompt_cond=moe_prompt_cond,
    ...
    def configure_optimizers(self):
        lr = self.learning_rate  #Basic learning rate = 5e-5
        params = list(self.model.parameters()) # Parameters of Unets
        if self.cond_stage_model.Type == 'semantic':
            params = params + list(self.cond_stage_model.parameters()) #Parameters of Visual2Text adapter
        #     params = params + list(self.transformer_decoder.parameters()) #Parameters of Enhancer
        #     params = params + list(self.transformer_encoder.parameters()) #Parameters of Enhancer
        parameters = [{'params': params, 'lr': lr//2},{'params': self.structcond_stage_model.parameters(), 'lr': lr*2}] # cond_stage_model: 5e-5, moe_prompt_cond:  2e-4           
        if self.learn_logvar:
            assert not self.learn_logvar
            print('Diffusion model optimizing logvar')
            params.append(self.logvar)
        opt = torch.optim.AdamW(parameters, lr=lr) #define the optimizer

        if self.use_scheduler: # Not use
        ...
        return opt # Here return the optimizer
