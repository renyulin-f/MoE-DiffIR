...
def compute_gating(k: int, probs: torch.Tensor, top_k_gates: torch.Tensor, top_k_indices: torch.Tensor):
    zeros = torch.zeros_like(probs) #(8192,2)
    gates = zeros.scatter(1, top_k_indices, top_k_gates) #(1024,8)
    ...
    expert_size = (gates > 0).long().sum(0) #8
    index_sorted_experts = nonzeros[_index_sorted_experts]
    batch_index = index_sorted_experts.div(k, rounding_mode='trunc') #(16384)
...
class MoE(nn.Module):
    ...
    def __init__(self, input_size, head_size, num_experts, k,
        ...
        self.head_size = head_size  #512
        self.bias = bias
        self.experts = ParallelExperts(num_experts, input_size, head_size, bias) #(8,256,512,False)
        self.output_experts = ParallelExperts(num_experts, head_size, input_size, bias) #
        self.k = min(k, self.num_experts)  #4
        self.cvloss = cvloss
        ...
        if True:
            ...
            self.f_gate = nn.Sequential(
                ...
                nn.Linear(input_size,
                          2 * num_experts if noisy_gating else num_experts,   #(256)->(32)
                          bias=False)
            )
...
class TaskMoE(MoE):
    ...
    def __init__(self,  input_size, head_size, num_experts, k, w_MI=0, limit_k=0, w_topk_loss=0.0, task_num=9, noisy_gating=True, gating_activation=None, **kwargs):
        ...
        self.limit_k = max(k, limit_k)#4
        #input_size = 256, head_size=64
    ...
    def top_k_gating(self, x, task_bh, bsz,skip_mask=None, sample_topk=0,noise_epsilon=1e-2):
        ...
        # clean_logits = self.f_gate[task_bh](x) #(1024,32)
        ...
        probs = torch.softmax(logits, dim=1) + 1e-4 #8192*8
        if skip_mask is not None:
            probs = torch.masked_fill(probs, skip_mask, 0)
       ...
       # top_k_indecis: [batch, K]
        ...
        batch_gates, batch_index, expert_size, gates, index_sorted_experts = \
            compute_gating(self.k, probs, top_k_gates, top_k_indices) #4,(8192,4),(8192,4),(2,0,1,5)
        self.expert_size = expert_size
        self.index_sorted_experts = index_sorted_experts
        self.batch_index = batch_index
        self.batch_gates = batch_gates
    ...
    def forward(self, x, task_bh, skip_mask=None, sample_topk=0, multiply_by_gates=True):
        # y_ = self.forward_(x, skip_mask, sample_topk, multiply_by_gates)
        bsz, length, emb_size = x.size() #8,256,1024
        x = x.reshape(-1, emb_size)
        if skip_mask is not None:
            skip_mask = skip_mask.view(-1, 1)
        loss = self.top_k_gating(x, task_bh, bsz,skip_mask,  sample_topk=sample_topk) #x=(8192,256)
        with open('output.txt', 'w') as file:
            # 遍历并将每个元素写入文件
            for element in self.batch_index:
                file.write(str(element.item()) + '\n')
        expert_inputs = x[self.batch_index] # (16384,256)
        
        with open('output8.txt', 'w') as file:
            # 遍历并将每个元素写入文件
            for element in self.batch_gates:
                file.write(str(element.item()) + '\n')
        expert_inputs = x[self.batch_index] # (16384,256)
        ...
        #     # 遍历并将每个元素写入文件
        #     for row in expert_inputs:
        #         for element in row:
        #             file.write(str(element.item()) + '\n')
        h = self.experts(expert_inputs, self.expert_size)  #self.expert_size=[8]
        # with open('output3.txt', 'w') as file:
        #     # 遍历并将每个元素写入文件
        #     for row in h:
        #         for element in row:
        #             file.write(str(element.item()) + '\n')
        h = self.activation(h)  #(16384,512)
        expert_outputs = self.output_experts(h, self.expert_size)

        if multiply_by_gates:
            expert_outputs = expert_outputs * self.batch_gates[:, None]  # (16384,256)*(16384)
        ...
        y = y.view(bsz, length, self.input_size) #(8,1024,256)
        # assert torch.allclose(y, y_)
        return y, loss
    ...
    def map(self, x, task_bh, skip_mask=None, sample_topk=0):
        ...
        encourages all experts to be approximately equally used across a batch.
        """
        bsz, length, emb_size = x.size() #(1,1024,256)
        x = x.reshape(-1, emb_size)
        # if skip_mask is not None:
        #     skip_mask = skip_mask.view(-1, 1)
        loss = self.top_k_gating(x, task_bh, bsz ,skip_mask, sample_topk=sample_topk)

        expert_inputs = x[self.batch_index] #(4096,256)
