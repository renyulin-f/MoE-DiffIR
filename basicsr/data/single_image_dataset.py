...
class SingleImageNPDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_sample = imfrombytes(img_bytes_sample, float32=True) #(256,256,3)

        latent_np = np.load(np_path) #(4,32,32)
        ...
        img_sample = img2tensor(img_sample, bgr2rgb=True, float32=True) #(3,256,256)
        ...
        return {'lq': img_lq, 'lq_path': lq_path, 'gt': img_gt, 'gt_path': gt_path, 'latent': latent_np, 'latent_path': np_path, 'sample': img_sample, 'sample_path': sample_path}
